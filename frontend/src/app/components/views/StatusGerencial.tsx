...
export default function StatusGerencial() {
  ...
  useEffect(() => {
    async function loadStatus() {
      setIsLoading(true);
      try {
        const { data: especialidadesData } = await supabase.from('vw_resumen_metrados_especialidad').select('*');

        let newRows: any[] = [];
        
        if (especialidadesData && especialidadesData.length > 0) {
          newRows = especialidadesData.map((row: any) => {
            const especialidad = row.especialidad_nombre;
            const ejecutado = row.total_ejecutado || 0;
            const presupuesto = ejecutado * 1.5; // Dummy for now
            return {
              especialidad,
              presupuesto,
              ejecutado,
        ...
        const newRadar = newRows.map((r: any) => ({
          especialidad: r.especialidad,
          avance: Math.min(100, Math.round(r.pct)),
          meta: 75, // Dummy meta
        }));

        setRows(newRows.length > 0 ? newRows : [{ especialidad: 'Estructuras', presupuesto: 100, ejecutado: 0, pct: 0 }]);
        setRadarData(newRadar.length > 0 ? newRadar : [{ especialidad: 'Estructuras', avance: 0, meta: 75 }]);
      } catch (err) {
        console.error(err);
      } finally {
        setIsLoading(false);
  ...
  const totalPct = totalPresupuesto > 0 ? (totalEjecutado / totalPresupuesto * 100).toFixed(1) : '0.0';
