...
async function runMigration() {
  ...
  if (!proys || proys.length === 0) {
    console.error("No se encontró ningún proyecto.");
    return;
  }
  ...
  // 3. Borrar Metrados (registro_metrados)
  console.log("Borrando registro_metrados...");
  // Truco para borrar todo sin error de null
  const { error: errMetrados } = await supabase.from('registro_metrados').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  if (errMetrados) console.error("Error borrando metrados:", errMetrados);

  // 4. Borrar Partidas (Preservando PC y ACT)
  console.log("Borrando partidas oficiales antiguas...");
  ...
  if (errPartidas) console.error("Error borrando partidas:", errPartidas);
